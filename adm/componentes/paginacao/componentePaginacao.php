<?php class CPaginacao
	{
		...
		private function comunicaComponenteTR($objConfiguracao, $str_caminhoComponente)
		{			
...
<table width="766" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td class="top_grupoRes_top_04" style="width:196px;"><span class="adm_fonteResTop_02">Ordenar por :</span><?php $this->comboOrdenacao($objConfiguracao);?>
		</td>
...
<?php return false;
		...
		public function mostraRegistroPaginadoComponenteTR($objConexao,$objConfiguracao,$query, $str_caminhoComponente, $int_codigo = '')
		{	
			if ($int_codigo != '')
			{
				if($this->comunicaComponenteTR($objConfiguracao, $str_caminhoComponente))
				{
					if($query != "")
					{
	...
	?>
					<script>
						function atribuiPaginacao($int_paginacao, $slc_totalRegistrosTela, $slc_ordenacao)
						{
							...
							$arrayNome 	= new Array ('"','', '"');
							$arrayValor = new Array ('',sequenceCrypt(document.getElementById('str_nomePagina').value, document.getElementById('codSec').value, false),'');
							var $nomePagina = retornaUrlAjax('', $arrayNome, $arrayValor);
							
							$arrayNome 	= new Array ('"','', '"');
							$arrayValor = new Array ('',<?php echo $int_codigo;?>,'');
						...
						function atribuiPaginacaoComboBox()
						{
							...
							$arrayNome 	= new Array ('int_paginacao','slc_totalRegistrosTela', 'slc_ordenacao','nomePagina', 'int_cod');
							$arrayValor = new Array (1, document.getElementById('slc_totalRegistrosTela').value, $valorOrdenacao, $nomePagina, $int_codigo);
...
<?php while (($this->contador <= $this->totalRegistros) and ($this->contador >= $this->primeiro and $this->contador <= $this->ultimo)) 
					{					
						cPaginacaoResultado($objConexao, $objConfiguracao,$this,$this->query);	
						$this->contador++;
					}
			...
			}else//####################################################################### COMPONENTE RESULTADOS SEM ID.
			{
				if($this->comunicaComponenteTR($objConfiguracao, $str_caminhoComponente))
				{
					if($query != "")
	...
	?>
					<script>
						...
						function atribuiPaginacaoComboBox()
						{
							...
							//concatena os nomes de variaveis aos respectivos valores
							$arrayNome 	= new Array ('int_paginacao','slc_totalRegistrosTela', 'slc_ordenacao','nomePagina');
							$arrayValor = new Array (1, document.getElementById('slc_totalRegistrosTela').value, $valorOrdenacao, $nomePagina);
...
?>
				<a href='#' onClick="atribuiPaginacao('1')">Primeira</a> - 
				<a href='#' onClick="atribuiPaginacao('<?php echo $anterior;?>')">Anterior</a> - <?php echo $pontosTras;?>
<?php }			
...
<?php }
				...
				{					
?>
					[ <a href='#' onClick="atribuiPaginacao('<?php echo $contador;?>')"><?php echo $contador;?></a> ]
<?php }
...
?>		  
				<?php echo $pontosFrente;?> - <a href='#' onClick='atribuiPaginacao("<?php echo $proxima;?>")'>Próxima</a> - <a href='#' onClick='atribuiPaginacao("<?php echo $this->totalPaginas;?>")'>Última</a>
<?php }
...
<?php }
		...
		private function comboOrdenacao($objConfiguracao)
		{
			$this->inicializarValores();
?>
...
<?php for($cont=0; $cont <sizeof($objConfiguracao->arrayOrdenacao); $cont++)
			{
				$selected = "";
				
				if($this->OrdenacaoRegistrosTela == $objConfiguracao->arrayOrdenacao[$cont+1])
				{
					$selected = "selected='selected'";
				}
?>
				<option value="<?php echo $objConfiguracao->arrayOrdenacao[$cont+1];?>" <?php echo $selected;?> title="<?php echo $objConfiguracao->arrayOrdenacao[$cont];?>"><?php echo $objConfiguracao->arrayOrdenacao[$cont];?></option>
<?php $cont++;
			}
?>
			</select>
...
<?php for($cont=0; $cont <sizeof($array); $cont++)
			{
				$selected = "";
				
				if($this->totalRegistrosTela == $array[$cont])
				{
					$selected = "selected='selected'";
				}
?>
				<option value="<?php echo $array[$cont];?>" <?php echo $selected;?>><?php echo $array[$cont];?></option>
<?php }
?>
			</select>
<?php }
