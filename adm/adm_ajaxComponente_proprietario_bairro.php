<?php header("Content-Type: text/html; charset=ISO-8859-1",true);
	...
	if (isset($_POST["id_bairro"]) || !empty($_POST["id_bairro"]))
	{
		if ($_POST["id_bairro"] != '')
		{
			$query = $objProprietario->comboLogradouro($objConexao, $_POST["id_bairro"]);
			$cont = 0;
			while($array = $objConexao->retornaArray($query))
			{
				...
				$combo .= ' title="'.$objProprietario->codifiStringBancoInterface($objConexao, ucwords($array['str_descricaologradouro'])).'" >';
				$combo .= $objProprietario->codifiStringBancoInterface($objConexao, ucwords($array['str_descricaologradouro'])).'</option>';
			}
		}else
		{
			$combo  = '<select name="slc_descricaoLogradouro" id="*" class="adm_formResCombo_01" style="width:125" title=\'Logradouro\' >';
